...
def concept_accuracy(random_seed: int, batch_size: int, latent_dim: int, model_name: str, test_fraction: float = 0.1,
                     model_dir: Path = Path.cwd()/"results/seer", data_dir: Path = Path.cwd()/"data/seer",
...
def global_explanations(random_seed: int, batch_size: int, latent_dim: int, plot: bool, model_name: str, test_fraction: float = 0.1,
                        model_dir: Path = Path.cwd()/"results/seer", data_dir: Path = Path.cwd()/"data/seer",
    ...
    for X_test, Y_test, C_test in tqdm(test_loader, unit="batch", leave=False):
        ...
        results_data += [["TCAR", label.item()] + [pred_concept[example_id] for pred_concept in pred_concepts]
                         for example_id, label in enumerate(Y_test)]
    results_df = pd.DataFrame(results_data, columns=["Method", "Class"]+[f"Grade {i+1}" for i in range(5)])
    logging.info(f"Saving results in {save_dir}")
...
def feature_importance(random_seed: int, batch_size: int, latent_dim: int, plot: bool, model_name: str, test_fraction: float = 0.1,
                       model_dir: Path = Path.cwd()/"results/seer", data_dir: Path = Path.cwd()/"data/seer",
    ...
    results_df = pd.DataFrame(results_data, columns=["Age", "PSA", "Positive Cores", "Examined Cores", "Clinical Stage",
                                                     "Gleason Scores"])
    logging.info(f"Saving results in {save_dir}")
    results_df.to_csv(save_dir/"metrics.csv", index=False)
    if plot:
