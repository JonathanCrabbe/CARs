...
def adversarial_robustness(random_seed: int, batch_size: int, latent_dim: int,
    ...
    test_set.transform = transforms.Compose([transforms.ToTensor()])
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=False)
    attacker = Attacker(model, 100, .1, device)

    logging.info("Now predicting concepts on the test set")
    for attack_prop in [0, .05, .1, .2, .5, .7, 1]:
        ...
        for X_test, y_test in tqdm(test_loader, unit="batch", leave=False):
            ...
            results_data += [[attack_prop*100, "TCAR", label.item()] + [int(car_pred[idx]) for car_pred in car_preds]
                             for idx, label in enumerate(y_test)]
            results_data += [[attack_prop*100, "TCAV", label.item()] + [int(cav_pred[idx] > 0) for cav_pred in cav_preds]
                             for idx, label in enumerate(y_test)]
            results_data += [[attack_prop*100, "True Prop.", label.item()] + [target[idx] for target in targets]
                             for idx, label in enumerate(y_test)]

    csv_path = save_dir / "metrics.csv"
    results_df = pd.DataFrame(results_data, columns=["Adversarial %", "Method", "Class"]+list(concept_to_class.keys()))
    results_df.to_csv(csv_path, index=False)
    scores_data = []
    ...
    for adv_pct, class_idx, concept, method in itertools.product(adv_pcts, classes, concepts, methods):
        attr = np.array(results_df.loc[(results_df.Class == class_idx) & (results_df.Method == method)
                                       & (results_df["Adversarial %"] == adv_pct)][concept])
        score = np.sum(attr) / len(attr)
        scores_data.append([adv_pct, method, class_idx, concept, score])
    scores_df = pd.DataFrame(scores_data, columns=["Adversarial %", "Method", "Class", "Concept", "Score"])
    corr_data = []
    for adv_pct in adv_pcts:
        tcar_scores = scores_df.loc[(scores_df.Method == "TCAR") & (scores_df["Adversarial %"] == adv_pct)]["Score"]
        tcav_scores = scores_df.loc[(scores_df.Method == "TCAV") & (scores_df["Adversarial %"] == adv_pct)]["Score"]
        true_scores = scores_df.loc[(scores_df.Method == "True Prop.") & (scores_df["Adversarial %"] == adv_pct)]["Score"]
        corr_data.append([adv_pct, "TCAR", np.corrcoef(tcar_scores, true_scores)[0, 1]])
        #corr_data.append([adv_pct, "TCAV", np.corrcoef(tcav_scores, true_scores)[0, 1]])
    corr_df = pd.DataFrame(corr_data, columns=["Adversarial %", "Method", "Correlation"])
    results_md = pd.pivot_table(data=corr_df, index="Adversarial %", columns="Method", aggfunc="mean", values="Correlation").to_markdown()
