...
if __name__ == "__main__":
    ...
    train, _ = torchtext.datasets.IMDB.splits(text_field=TEXT, label_field=Label, train='train', test='test', root='data/')
    ...
    positive_reps_train, positive_reps_test = torch.split(positive_reps, split_size_or_sections=[90, 30])
    negative_reps_train, negative_reps_test = torch.split(negative_reps, split_size_or_sections=[90, 30])
    ...
    print(f"Accuracy of CAR classifier for the positive adjective concept: {accuracy_score(Y_test, car.predict(H_test)):.2g}")
